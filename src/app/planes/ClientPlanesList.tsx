...
export default function ClientPlanesList({ 
  ...
  const handleDeleteCiclo = async (id: string, nombre: string) => {
    if (confirm(`¿Estás seguro de borrar el plan "${nombre}"? Los hábitos dentro de este plan NO se borrarán.`)) {
      await (supabase.from('ciclos') as any).delete().eq('id', id);
      router.refresh();
    }
  };
  ...
  const handleBorrarDefinitivo = async (id: string, nombre: string) => {
    if (confirm(`¿BORRAR DEFINITIVAMENTE "${nombre}"? Se perderá todo su historial y estadísticas para siempre.`)) {
      await (supabase.from('habitos') as any).delete().eq('id', id);
      router.refresh();
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Tabs Selector */}
      <div className="flex bg-background p-1 rounded-2xl border border-background">
        <button
          onClick={() => setActiveTab('planes')}
          className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === 'planes' ? 'bg-charcoal text-neon-green shadow-lg border border-background' : 'text-gray-500 hover:text-gray-300'}`}
        >
        ...
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === 'baul' ? 'bg-charcoal text-neon-green shadow-lg border border-background' : 'text-gray-500 hover:text-gray-300'}`}
        >
          <Archive className="w-4 h-4" />
          El Baúl
          {initialArchivados.length > 0 && (
            <span className="bg-neon-green/20 text-neon-green text-[10px] px-1.5 py-0.5 rounded-full">
        ...
        {activeTab === 'planes' ? (
          <div className="space-y-4">
            {initialCiclos.length === 0 ? (
              <div className="text-center py-12 px-8 bg-charcoal rounded-3xl border border-background border-dashed">
                <Target className="w-12 h-12 text-neon-green/20 mx-auto mb-4" />
                <h3 className="text-foreground font-bold mb-2">No hay planes creados</h3>
              ...
              initialCiclos.map((ciclo) => {
                ...
                let cardColors = 'bg-charcoal border-background hover:border-gray-500/50';
                
                if (status === 'ACTIVO') {
                  statusColors = 'bg-neon-green/20 text-neon-green border-neon-green/50 shadow-[0_0_10px_rgba(57,255,20,0.2)]';
                  cardColors = 'bg-charcoal border-neon-green/50 hover:shadow-[0_0_20px_rgba(57,255,20,0.15)]';
                } else if (status === 'PRÓXIMO') {
                  statusColors = 'bg-electric-blue/20 text-electric-blue border-electric-blue/50';
                  cardColors = 'bg-charcoal border-background hover:border-electric-blue/50';
                }
                  ...
                  <div key={id} className={`p-5 rounded-2xl border transition-all duration-300 relative group overflow-hidden ${cardColors}`}>
                    ...
                    <div className="flex justify-between items-start relative z-10">
                      <div>
                        ...
                        <h2 className="text-xl font-extrabold text-foreground mt-3">{nombre}</h2>
          ...
          <div className="space-y-4">
            {initialArchivados.length === 0 ? (
              <div className="text-center py-12 px-8 bg-charcoal rounded-3xl border border-background border-dashed">
                <Archive className="w-12 h-12 text-neon-green/20 mx-auto mb-4" />
                <h3 className="text-foreground font-bold mb-2">El Baúl está vacío</h3>
              ...
              initialArchivados.map((habito) => (
                <div key={habito.id} className="p-4 bg-charcoal border border-background rounded-2xl flex items-center justify-between hover:border-gray-500/50 transition-all">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-background rounded-xl flex items-center justify-center text-2xl shadow-inner border border-background/50">
                      {habito.icono}
                    </div>
                    <div>
                      <h3 className="text-foreground font-bold text-lg leading-tight">{habito.nombre}</h3>
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      ...
                      className="p-3 bg-neon-green/10 text-neon-green border border-neon-green/30 rounded-xl hover:bg-neon-green/20 transition-all shadow-sm"
